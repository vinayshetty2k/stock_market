...
class Nse(AbstractBaseExchange):
    ...
    def __init__(self):
        ...
        self.preopen_all_url = "https://www1.nseindia.com/live_market/dynaContent/live_analysis/pre_open/all.json"
    ...
    def get_json_data(self, json_url, as_json=False):
        return self._get_json_response_from_url(json_url, as_json)
    ...
    def get_preopen_all(self, as_json=False):
        return self._get_json_response_from_url(self.preopen_all_url, as_json)
    ...
    def download_bhavcopy(self, d, instrument_type):
        url = self.get_bhavcopy_url(d)
        filename = self.get_bhavcopy_filename(d)
        if instrument_type == "/DERIVATIVES/":
            new_filename = ''.join(['fo', filename[2:]])
            url = url.replace('/EQUITIES/',instrument_type)
    ...
    def save_bhavcopy(self, d, instrument_type, path):
        url = self.get_bhavcopy_url(d)
        filename = self.get_bhavcopy_filename(d)
        if instrument_type == "/DERIVATIVES/":
            new_filename = ''.join(['fo', filename[2:]])
